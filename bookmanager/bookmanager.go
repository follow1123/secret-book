...
import (
	...
	"github.com/follow1123/secret-book/encryptor"
...
func New(bookPath string, password string) (*BookManager, error) {
	encryptor, err := encryptor.New(bookPath, password)
	if err != nil {
		return nil, fmt.Errorf("create encryptor error:\n\t%w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("get data error:\n\t%w", err)
	}
...
func (m *BookManager) Save() error {
	...
	if err := m.encryptor.Update(data); err != nil {
		return err
	}

	if err := m.encryptor.Save(); err != nil {
		return err
	}
	return nil
}
...
func DefaultSecretsFile() string {
	...
	return filepath.Join(path, "secrets")
}
